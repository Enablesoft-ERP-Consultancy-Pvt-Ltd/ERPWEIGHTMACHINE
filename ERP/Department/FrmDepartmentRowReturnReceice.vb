...
Public Class FrmDepartmentRowReturnReceice
    ...
    Private Sub FrmDepartmentRowIssue_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        Dim str As String = "select Distinct CI.CompanyId,Companyname from Companyinfo CI " & _
                    ...
                    " Where a.TYPEFLAG = 2 And a.MasterCompanyID = " & VarMasterCompanyID & "  " & _
    ...
    Private Sub DDprocess_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles DDprocess.SelectedIndexChanged
        If DDprocess.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            ...
            " Where a.TYPEFLAG = 2 And a.CompanyID = " & DDcompany.SelectedValue & "" & _
    ...
    Protected Sub FillIssueNo()
        If DDcompany.SelectedIndex <> -1 And DDDepartment.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            ...
            " Where a.TYPEFLAG = 2 And a.CompanyID = " & DDcompany.SelectedValue & " And a.BranchID = " & DDBranchName.SelectedValue & " And a.ProcessID = " & DDprocess.SelectedValue & " And " & _
            " a.DepartmentID = " & DDDepartment.SelectedValue & " And a.MasterCompanyID = " & VarMasterCompanyID & " " & _
            " Order By a.IssueOrderID Desc"

            NewComboFill(ddOrderNo, str)
        End If
    End Sub
    Private Sub DDissueNo_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles ddOrderNo.SelectedIndexChanged
        If DDcompany.SelectedIndex <> -1 And DDDepartment.SelectedIndex <> -1 And ddOrderNo.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            ...
            " Where TypeFlag = 2 And TranType=0 And IssueOrderID = " & ddOrderNo.SelectedValue & " And MasterCompanyId = " & VarMasterCompanyID & " " & _
    ...
    Private Sub DDReceiveNo_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles DDIssueNo.SelectedIndexChanged
        If DDcompany.SelectedIndex <> -1 And DDDepartment.SelectedIndex <> -1 And ddOrderNo.SelectedIndex <> -1 And DDIssueNo.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            Dim str As String = "Select Distinct VF.CATEGORY_ID, VF.CATEGORY_NAME " & _
                ...
                " Where a.TYPEFLAG = 2 And a.PRMID = " & DDIssueNo.SelectedValue & "" & _
    ...
    Private Sub ddCatagory_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles ddCatagory.SelectedIndexChanged
        If ddCatagory.SelectedIndex <> -1 And ddOrderNo.SelectedIndex <> -1 And DDprocess.SelectedIndex <> -1 And Varcombovalue <> 0 Then
    ...
    Private Sub DDShape_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles DDShape.SelectedIndexChanged
        If DDcompany.SelectedIndex <> -1 And ddCatagory.SelectedIndex <> -1 And ddOrderNo.SelectedIndex <> -1 And dditemname.SelectedIndex <> -1 And DDShape.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            FindFinishedID()
            Dim str As String = " Select Distinct VF.SizeID, VF.SizeFt " & _
                            ...
                            " Where a.TYPEFLAG = 2 And a.CompanyID = " & DDcompany.SelectedValue & " And a.PRMID = " & DDIssueNo.SelectedValue & " " & _
                            " Order By VF.SizeFt"
    ...
    Private Sub ddlshade_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles DDShadeColor.SelectedIndexChanged
        If dditemname.SelectedIndex <> -1 And DDQuality.SelectedIndex <> -1 And DDShadeColor.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            ...
            Dim str As String = "Select Distinct b.LotNo, b.LotNo " & _
                " From DEPARTMENTRAWISSUEMASTER a(Nolock) " & _
                " JOIN DEPARTMENTRAWISSUETRAN b(Nolock) on b.PRMID = a.PRMID  " & _
                " Where a.TYPEFLAG = 2 And a.PRMID = " & DDIssueNo.SelectedValue & " And b.ITEM_FINISHED_ID = " & TxtFinishedID.Text & " " & _
                " Select Distinct GM.GodownID,GM.GodownName  " & _
                " From GodownMaster GM JOIN Godown_Authentication GA ON GM.GodownId=GA.GodownId and GA.UserId=" & varuserid & " and GA.MasterCompanyId=" & VarMasterCompanyID & " " & _
                " Order By GM.GodownName"

            Dim ds As DataSet = SqlHelper.ExecuteDataset(Con, CommandType.Text, str)

            NewComboFillWithDs(DDLotno, ds, 0)
            NewComboFillWithDs(DDGodown, ds, 1)
        End If
    End Sub
    ...
    Protected Sub FillStockRecQty()
        Dim str As String = " Select IsNull(Sum(b.Qty), 0) IssQty " & _
                " From DEPARTMENTRAWISSUEMASTER a(Nolock) " & _
                " JOIN DEPARTMENTRAWISSUETRAN b(Nolock) on b.PRMID = a.PRMID  " & _
                " Where a.TranType = 0 And a.TypeFlag = 2 And a.PRMID = " & DDIssueNo.SelectedValue & " And b.ITEM_FINISHED_ID = " & TxtFinishedID.Text & " " & _
        ...
        str = str + "  Select IsNull(Sum(b.Qty), 0) RecQty  " & _
                " From DEPARTMENTRAWISSUEMASTER a(Nolock) " & _
                " JOIN DEPARTMENTRAWISSUETRAN b(Nolock) ON b.PRMID = a.PRMID  " & _
                " Where a.TranType = 1 And a.TypeFlag = 2 And a.IssuePRMID = " & DDIssueNo.SelectedValue & " And b.ITEM_FINISHED_ID = " & TxtFinishedID.Text & " " & _
                    " And b.LotNo = '" & DDLotno.Text & "'"
        If DDTagNo.Visible = True Then
            str = str + " And b.TAGNO = '" & DDTagNo.Text & "'"
        End If
        ...
        TxtPendingQty.Text = (Math.Round(Convert.ToDouble(ds.Tables(0).Rows(0)("IssQty")) - Convert.ToDouble(ds.Tables(1).Rows(0)("RecQty")), 3)).ToString()
    End Sub
    ...
    Protected Sub FillBinNo()
        If dditemname.SelectedIndex <> -1 And DDQuality.SelectedIndex <> -1 And DDShadeColor.SelectedIndex <> -1 And DDGodown.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            ...
            param(0) = New SqlParameter("@Godownid", SqlDbType.Int)
            param(1) = New SqlParameter("@Item_finished_id", SqlDbType.Int)
            param(2) = New SqlParameter("@New_Edit", SqlDbType.Int)

            param(0).Value = DDGodown.SelectedValue
            param(1).Value = TxtFinishedID.Text
            param(2).Value = 0

            Dim ds As DataSet = SqlHelper.ExecuteDataset(Con, CommandType.StoredProcedure, "FILL_BINNO", param)
    ...
    Protected Sub FillGodown()
        If dditemname.SelectedIndex <> -1 And DDQuality.SelectedIndex <> -1 And DDShadeColor.SelectedIndex <> -1 And DDLotno.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            ...
            " Where a.TRANTYPE = 1 And a.TYPEFLAG = 1 And a.PRMID = " & DDIssueNo.SelectedValue & " " & _
            " Order By GM.GodownName "
    ...
    Private Sub btnsave_Click(sender As System.Object, e As System.EventArgs) Handles btnsave.Click
        ...
        Try
            ...
            param(4).Value = ddOrderNo.SelectedValue
            ...
            param(25).Value = DDIssueNo.SelectedValue
