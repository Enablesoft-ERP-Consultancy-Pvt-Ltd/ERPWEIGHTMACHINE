...
Public Class FrmNextProcessReceive
    ...
    Private Sub frmsampleyarndyeing_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        NewComboFillWithDs(DDProcessName, ds, 2)
        If DDProcessName.Items.Count > 0 Then DDProcessName.SelectedIndex = 0

        NewComboFillWithDs(DDCategory, ds, 3)
        DDPartyName.Focus()
    End Sub
    ...
    Private Sub DDPartyName_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles DDPartyName.SelectedIndexChanged
        If DDPartyName.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            NewComboFill(DDIssueNo, "Select Distinct a.ISSUEORDERID, a.ChallanNo " & _
            " From PROCESS_ISSUE_MASTER_" & DDProcessName.SelectedValue & " a" & _
            " JOIN PROCESS_ISSUE_DETAIL_" & DDProcessName.SelectedValue & " b ON b.ISSUEORDERID = a.ISSUEORDERID And b.PQTY > 0 " & _
            " JOIN Employee_ProcessOrderNo EPO ON EPO.ProcessId = " & DDProcessName.SelectedValue & " And EPO.IssueOrderId = a.ISSUEORDERID And " & _
            " EPO.IssueDetailId = b.ISSUE_DETAIL_ID And EPO.Empid = " & DDPartyName.SelectedValue & "" & _
            " Where a.STATUS = 'Pending' And a.COMPANYID = " & DDCompanyName.SelectedValue & " " & _
    ...
    Private Sub DDCategory_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles DDCategory.SelectedIndexChanged
        If DDCategory.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            Dim stritem As String = "Select Distinct VF.ITEM_ID, VF.ITEM_NAME " & _
            " From PROCESS_ISSUE_DETAIL_" & DDProcessName.SelectedValue & " a(Nolock) " & _
            " JOIN V_FinishedItemDetail VF ON VF.ITEM_FINISHED_ID = a.ITEM_FINISHED_ID " & _
            " Where a.PQTY > 0 And a.ISSUEORDERID = " & DDIssueNo.SelectedValue & " And VF.MasterCompanyId = " & VarMasterCompanyID & " " & _
            " Order By VF.ITEM_NAME " & _
            " SELECT PARAMETER_ID FROM ITEM_CATEGORY_PARAMETERS(Nolock) where CATEGORY_ID=" & DDCategory.SelectedValue
    ...
    Private Sub DDItem_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles DDItem.SelectedIndexChanged
        If DDItem.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            Dim str As String = "Select Distinct VF.QualityID, VF.QualityName " & _
            " From PROCESS_ISSUE_DETAIL_" & DDProcessName.SelectedValue & " a(Nolock) " & _
            " JOIN V_FinishedItemDetail VF ON VF.ITEM_FINISHED_ID = a.ITEM_FINISHED_ID " & _
            " Where a.PQTY > 0 And a.ISSUEORDERID = " & DDIssueNo.SelectedValue & " And VF.MasterCompanyId = " & VarMasterCompanyID & " " & _
                " And VF.CATEGORY_ID = " & DDCategory.SelectedValue & " And VF.ITEM_ID = " & DDItem.SelectedValue

            Dim ds As DataSet = SqlHelper.ExecuteDataset(Con, CommandType.Text, str)

            NewComboFillWithDs(DDQuality, ds, 0)
        End If
    End Sub
    ...
    Private Sub DDQuality_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles DDQuality.SelectedIndexChanged
        If DDQuality.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            Dim str As String = "Select Distinct VF.DesignID, VF.DesignName " & _
            " From PROCESS_ISSUE_DETAIL_" & DDProcessName.SelectedValue & " a(Nolock) " & _
            " JOIN V_FinishedItemDetail VF ON VF.ITEM_FINISHED_ID = a.ITEM_FINISHED_ID " & _
            " Where a.PQTY > 0 And a.ISSUEORDERID = " & DDIssueNo.SelectedValue & " And VF.MasterCompanyId = " & VarMasterCompanyID & " " & _
                " And VF.CATEGORY_ID = " & DDCategory.SelectedValue & " And VF.ITEM_ID = " & DDItem.SelectedValue & " And VF.QualityID = " & DDQuality.SelectedValue & " Order By VF.DesignName " & _
            " Select Distinct VF.ColorId, VF.ColorName " & _
            " From PROCESS_ISSUE_DETAIL_" & DDProcessName.SelectedValue & " a(Nolock) " & _
            " JOIN V_FinishedItemDetail VF ON VF.ITEM_FINISHED_ID = a.ITEM_FINISHED_ID " & _
            " Where a.PQTY > 0 And a.ISSUEORDERID = " & DDIssueNo.SelectedValue & " And VF.MasterCompanyId = " & VarMasterCompanyID & " " & _
                " And VF.CATEGORY_ID = " & DDCategory.SelectedValue & " And VF.ITEM_ID = " & DDItem.SelectedValue & " And VF.QualityID = " & DDQuality.SelectedValue & " Order By VF.ColorName " & _
            " Select Distinct VF.ShapeId, VF.ShapeName " & _
            " From PROCESS_ISSUE_DETAIL_" & DDProcessName.SelectedValue & " a(Nolock) " & _
            " JOIN V_FinishedItemDetail VF ON VF.ITEM_FINISHED_ID = a.ITEM_FINISHED_ID " & _
            " Where a.PQTY > 0 And a.ISSUEORDERID = " & DDIssueNo.SelectedValue & " And VF.MasterCompanyId = " & VarMasterCompanyID & " " & _
                " And VF.CATEGORY_ID = " & DDCategory.SelectedValue & " And VF.ITEM_ID = " & DDItem.SelectedValue & " And VF.QualityID = " & DDQuality.SelectedValue & " Order By VF.ShapeName " & _
            " Select Distinct VF.ShadecolorId, VF.ShadeColorName " & _
            " From PROCESS_ISSUE_DETAIL_" & DDProcessName.SelectedValue & " a(Nolock) " & _
            " JOIN V_FinishedItemDetail VF ON VF.ITEM_FINISHED_ID = a.ITEM_FINISHED_ID " & _
            " Where a.PQTY > 0 And a.ISSUEORDERID = " & DDIssueNo.SelectedValue & " And VF.MasterCompanyId = " & VarMasterCompanyID & " " & _
                " And VF.CATEGORY_ID = " & DDCategory.SelectedValue & " And VF.ITEM_ID = " & DDItem.SelectedValue & " And VF.QualityID = " & DDQuality.SelectedValue & " Order By VF.ShadeColorName "
            Dim ds As DataSet = SqlHelper.ExecuteDataset(Con, CommandType.Text, Str)
    ...
    Private Sub DDShape_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles DDShape.SelectedIndexChanged
        If DDSize.Visible = True And DDShape.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            Dim str As String = " Select Distinct VF.SizeID, Case When a.UNITID = 1 Then VF.SizeMtr Else Case When a.UnitID = 6 Then VF.SizeInch Else VF.SizeFt End End Size " & _
            " From PROCESS_ISSUE_MASTER_" & DDProcessName.SelectedValue & " a(Nolock) " & _
            " JOIN PROCESS_ISSUE_DETAIL_" & DDProcessName.SelectedValue & " b(Nolock) ON b.ISSUEORDERID = a.ISSUEORDERID And b.PQTY > 0 " & _
            " JOIN V_FinishedItemDetail VF ON VF.ITEM_FINISHED_ID = b.ITEM_FINISHED_ID  " & _
            " Where a.ISSUEORDERID = " & DDIssueNo.SelectedValue & " And VF.MasterCompanyId = " & VarMasterCompanyID & " " & _
            " And VF.CATEGORY_ID = " & DDCategory.SelectedValue & " And VF.ITEM_ID = " & DDItem.SelectedValue & " And VF.QualityID = " & DDQuality.SelectedValue & " " & _
    ...
    Private Sub btnsave_Click(sender As System.Object, e As System.EventArgs) Handles btnsave.Click
        ...
        End If
        If Validtxtbox(TxtReceiveQty) = False Then Exit Sub
        ...
        Try
            ...
            arr(0) = New SqlParameter("@Process_Rec_ID", SqlDbType.Int)
            ...
            arr(3) = New SqlParameter("@ProcessID", DDProcessName.SelectedValue)
            arr(4) = New SqlParameter("@EmpID", DDPartyName.SelectedValue)
            arr(5) = New SqlParameter("@IssueOrderID", DDIssueNo.SelectedValue)
            arr(6) = New SqlParameter("@ReceiveDate", TxtDate.Text)
            ...
            arr(7) = New SqlParameter("@FinishedID", varfinishedid)
            arr(8) = New SqlParameter("@ChallanNo", SqlDbType.VarChar, 50)
            arr(8).Direction = ParameterDirection.InputOutput
            arr(8).Value = TxtChallanNo.Text
            arr(9) = New SqlParameter("@CALTYPE", 0)
            arr(10) = New SqlParameter("@ReceiveQty", TxtReceiveQty.Text)
            arr(11) = New SqlParameter("@Weight", TxtWeight.Text)

            arr(12) = New SqlParameter("@UserID", varuserid)
            arr(13) = New SqlParameter("@MasterCompanyID", VarMasterCompanyID)
            arr(14) = New SqlParameter("@msg", SqlDbType.VarChar, 100)
            arr(14).Direction = ParameterDirection.Output

            SqlHelper.ExecuteNonQuery(Tran, CommandType.StoredProcedure, "Pro_SaveNextProcessReceiveQtyWise", arr)
            hnid = arr(0).Value.ToString()
            TxtChallanNo.Text = arr(8).Value.ToString()
            MsgBox(arr(14).Value.ToString())
            Tran.Commit()
            Fillgrid()
            TxtReceiveQty.Text = ""
            TxtWeight.Text = ""
    ...
    Protected Sub Fillgrid()
        DG.Rows.Clear()
        Dim str As String = "Select a.Process_Rec_ID, b.Process_Rec_Detail_ID, VF.ITEM_NAME + ' ' + VF.QualityName + ' ' + VF.QualityName + ' ' + VF.DesignName + ' ' + VF.ColorName + ' ' + VF.ShapeName + ' ' + " & _
                " Case When a.UnitID = 1 Then VF.SizeMtr Else Case When a.UnitID = 6 Then VF.SizeInch Else VF.SizeFt End End [Description], b.AREA * b.Qty Area, b.[Weight], b.Qty, b.Rate, b.Amount " & _
                " From PROCESS_RECEIVE_MASTER_" & DDProcessName.SelectedValue & " a(Nolock) " & _
                " JOIN PROCESS_RECEIVE_DETAIL_" & DDProcessName.SelectedValue & " b(Nolock) ON b.PROCESS_REC_ID = a.PROCESS_REC_ID " & _
                " JOIN V_FinishedItemDetail VF(Nolock) ON VF.ITEM_FINISHED_ID = b.ITEM_FINISHED_ID " & _
                " Where a.Process_Rec_ID = " & hnid
        Dim ds As DataSet = SqlHelper.ExecuteDataset(Con, CommandType.Text, str)
    ...
    Private Sub btnpreview_Click(sender As System.Object, e As System.EventArgs) Handles btnpreview.Click
        ...
        Dim str As String = "Select Ci.CompanyId,BM.BranchName CompanyName,BM.BranchAddress CompAddr1,'' CompAddr2, '' CompAddr3,BM.PhoneNo CompTel,'' CompFax,CI.GSTNo " & _
        " ,EI.Empname,Ei.Empaddress as address,'' as Address2,'' as Address3,'' as Mobile,Ei.EMPGSTIN as Empgstin,PID.issueorderid " & _
        " ,PIM.Receivedate,(select PROCESS_NAME From PROCESS_NAME_MASTER Where PROCESS_NAME_ID=" & DDProcessName.SelectedValue & ") as Job, " & _
        " Vf.QualityName,Vf.designName,Vf.ColorName,Case When Vf.shapeid=1 Then '' Else Left(vf.shapename,1) End  as Shapename, " & _
        " PID.Width+' x ' +PID.Length as Size,PID.qty,PID.Qty*PID.area as Area,PIM.UnitId,PID.Rate,PID.Amount,PID.Process_Rec_Detail_Id,PIM.challanNo, " & _
        " (Select * from [dbo].[Get_StockNoNext_Receive_Detail_Wise](PID.process_rec_detail_id," & DDProcessName.SelectedValue & ",PID.issue_detail_id)) TStockNo,PID.Item_Finished_Id " & _
        " ,'' FolioNo, ISNULL(PID.stockNoRemarks,'') as StockNoRemarks,ISNULL(PID.ActualWidth,'') as ActualWidth,ISNULL(PID.ActualLength,'') as ActualLength,isnull(PID.Weight,0) as Weight " & _
        " From PROCESS_Receive_MASTER_" & DDProcessName.SelectedValue & " PIM  " & _
        " Join PROCESS_Receive_DETAIL_" & DDProcessName.SelectedValue & " PID on PIM.Process_Rec_Id=PID.Process_Rec_Id " & _
        " Join BranchMaster BM(Nolock) ON BM.ID = PIM.BranchID  " & _
        " join CompanyInfo CI on PIM.Companyid=CI.CompanyId " & _
        " cross apply(select * From dbo.F_GetJobReceiveEmployeeDetail(" & DDProcessName.SelectedValue & ",PIM.Process_rec_id)) EI " & _
        " inner join V_FinishedItemDetail vf on PID.Item_finished_id=vf.ITEM_FINISHED_ID " & _
        " Where PIM.Process_rec_id=" & hnid & " order by Process_rec_detail_id "
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            sReportName = (My.Application.Info.DirectoryPath & "\Reports\RptNextReceiveNew2.rpt")
            dspath = (My.Application.Info.DirectoryPath & "\ReportSchema\RptNextReceiveNew2.xsd")
