...
Public Class frmmottelingreturn
    ...
    Private Sub DDPartyName_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles DDPartyName.SelectedIndexChanged
        ...
        If DDCompanyName.SelectedIndex <> -1 And DDProcessName.SelectedIndex <> -1 And DDPartyName.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            Dim str As String = "select ID,IssueNo + ' # ' +REPLACE(CONVERT(nvarchar(11),IssueDate,106),' ','-') as IssueNo From MOTTELINGISSUEMASTER Where Companyid=" & DDCompanyName.SelectedValue & " and processid=" & DDProcessName.SelectedValue & " and empid=" & DDPartyName.SelectedValue
    ...
    Protected Sub FillIssueDetail()
        ...
        If DDissueno.SelectedIndex <> -1 And Varcombovalue <> 0 Then
            Dim str As String = "select dbo.F_getItemDescription(YT.iFinishedid,YT.iflagsize) as ItemDescription," & _
                       "U.UnitName,YT.Lotno,YT.Tagno,Round(yt.IssueQty,3) as IssueQty, isnull(YRQ.retqty,0) as Returnedqty,YT.ifinishedid,YT.Unitid,YT.iflagsize as flagsize,YM.ID,yt.Detailid " & _
                       "from MottelingIssueMaster YM inner join MottelingIssueDetail YT on YM.ID = YT.MasterId " & _
    ...
    Private Sub FillReturnDetails()
        ...
        Dim str As String = "select Ym.id,Yt.detailid,YM.ReturnNo,dbo.F_getItemDescription(YT.Item_Finished_id,YT.flagsize) as ItemDescription," & _
                        ...
                        "from Mottelingreturnmaster YM inner join MottelingReturnDetail YT on YM.ID = YT.MasterId " & _
                        "inner join Unit U on YT.Unitid=U.UnitId " & _
    ...
    Private Sub btnpreview_Click(sender As System.Object, e As System.EventArgs) Handles btnpreview.Click
        Dim str As String = "select  * from [V_MOTTELINGRETURN] Where id=" & reportid
        Dim ds As DataSet = SqlHelper.ExecuteDataset(Con, CommandType.Text, str)
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            sReportName = (My.Application.Info.DirectoryPath & "\Reports\rptmottelingreturn.rpt")

            dspath = (My.Application.Info.DirectoryPath & "\ReportSchema\rptmottelingreturn.xsd")
